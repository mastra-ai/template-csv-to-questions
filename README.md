...
> **🎯 Key Learning**: This template shows how to use large context window models (Anthropic GPT-4.1 Mini) as a "summarization layer" to compress large CSV datasets into focused summaries, enabling efficient downstream processing without hitting token limits.
...
1. **Input**: CSV URL
2. **Download & Summarize**: Fetch CSV, parse data, and generate AI summary using Anthropic GPT-4.1 Mini
...
1. **Use a large context window model** (Anthropic GPT-4.1 Mini) to digest the full dataset
...
- ✅ **Large Context Window**: Uses Anthropic GPT-4.1 Mini to handle large datasets efficiently
